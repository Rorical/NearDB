...
// This is copied from https://github.com/ekzhu/minhash-lsh/blob/master/minhash.go
...
func CompHash(taglist []string, size int) Point {
	hashfun := NewMinhash(1145141919182, size)
	for _, tagbyte := range StringListToBytesList(taglist) {
		hashfun.Push(tagbyte)
	}
	point := make(Point, size)
	for i, p := range hashfun.Signature() {
		point[i] = float64(p)
	}
	return point
}